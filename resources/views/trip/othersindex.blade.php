<x-app-layout>
	<x-slot name="title">{{ $user->name }}の投稿一覧</x-slot>
	
	<div class="container mx-auto p-4">
		<div class="flex justify-between items-center border-b pb-2">
			<div class="text-xl font-bold">{{ $user->name }}</div>
			<div class="mt-4 text-lg">{{ Auth::user()->intorduce }}</div>
		</div>
			
		<div class="flex mt-4">
			<!-- Sidebar -->
			<div class="w-1/5 border-r pr-4">
				<h2 class="text-2xl font-bold mb-4">{{ $user->name }}の旅行先</h2>
				<div>
					...
					@foreach($trips as $trip)
						@if($trip->status == 1)
							<div class="items-center border p-4 bg-white rounded-md shadow-sm mb-4">
								<div id="title_{{ $trip->id }}" class="text-lg font-bold mb-1">{{ $trip->title }}</div>
